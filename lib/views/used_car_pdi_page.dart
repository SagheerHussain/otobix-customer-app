...
import 'package:otobix_customer_app/controllers/used_car_pdi_controller.dart';
...
class UsedCarPdiPage extends StatelessWidget {
  UsedCarPdiPage({super.key});
  ...
  final UsedCarPdiController c = Get.put(UsedCarPdiController());

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: AppColors.white,
      appBar: AppBarWidget(title: 'Used Car PDI'),
      body: SafeArea(
        child: Column(
          children: [
  ...
  // ========================= PAGE 1 =========================
  Widget _buildPageOne(BuildContext context, UsedCarPdiController c) {
    return Padding(
                          ...
                          Obx(
                            () => _buildDropdown(
                              ...
                              value: c.selectedMake.value.isEmpty
                                  ? null
                                  : c.selectedMake.value,
                              onTap: c.isMakeModelLocked.value
                                  ? () {}
                                  : c.onTapMake,
                            ),
                          ),
  ...
  Widget _buildRegField(UsedCarPdiController c) {
    return Container(
      height: 40,
      padding: const EdgeInsets.symmetric(horizontal: 14),
      decoration: BoxDecoration(
      ...
      child: Row(
        children: [
          Expanded(
            child: TextField(
              controller: c.registrationNumberController,
              ...
              onChanged: (value) {
                c.registrationNumberController.value = TextEditingValue(
          ...
          Obx(() {
            final loading = c.isMakeModelFetchLoading.value;
            return InkWell(
              onTap: loading ? null : c.fetchMakeAndModelFromAttestr,
              borderRadius: BorderRadius.circular(18),
              child: Container(
                height: 28,
                padding: const EdgeInsets.symmetric(horizontal: 12),
                decoration: BoxDecoration(
                  color: _titleBlue.withOpacity(0.10),
                  borderRadius: BorderRadius.circular(18),
                ),
                alignment: Alignment.center,
                child: loading
  ...
  Widget _buildRegField1(UsedCarPdiController c) {
    return Container(
      height: 40,
      padding: const EdgeInsets.symmetric(horizontal: 14),
      decoration: BoxDecoration(
  ...
  // ========================= PAGE 2 =========================
  Widget _buildPageTwo(BuildContext context, UsedCarPdiController c) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 14),
      child: Container(
        padding: const EdgeInsets.all(14),
  ...
  Widget _buildPincodeFieldLikeDropdown(UsedCarPdiController c) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
  ...
  // ✅ Date row (rebuild guaranteed)
  Widget _buildDateRow(UsedCarPdiController c) {
    return Obx(() {
  ...
  Widget _buildTimeSlots(UsedCarPdiController c) {
    return Obx(
      () => Wrap(
        spacing: 10,
        runSpacing: 10,
  ...
  Widget _buildTypeBox({
    ...
    required UsedCarPdiController c,
  ...
  // ========================= PAGE 3 =========================
  Widget _buildPageThree(UsedCarPdiController c) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 14),
      child: Container(
        padding: const EdgeInsets.all(14),
        ...
        child: Container(
          ...
          child: Row(
            children: [
              ...
              Icon(
                locked ? Icons.lock_rounded : Icons.arrow_drop_down_rounded,
                color: Color(0xFF23324A),
              ),
            ],
          ),
  ...
  Widget _buildYesNo(UsedCarPdiController c) {
    final selected = c.serviceHistoryYes.value;

    Widget box(bool isYes) {
      final checked = selected == isYes;
      return InkWell(
    ...
    return Column(
      children: [
        Row(children: [box(true), const SizedBox(width: 40), box(false)]),
        Obx(() {
          final bool modelSelected = c.selectedModel.value.isNotEmpty;
          final bool yesSelected = c.serviceHistoryYes.value == true;
          final bool historyTotalIsZero = c.serviceHistoryTotal.value == 0.0;
          final bool loaded = !c.isFetchPdiPriceLoading.value;

          if (modelSelected && yesSelected && historyTotalIsZero && loaded) {
            return Column(
              children: [
                const SizedBox(height: 10),
                Text(
                  'Service history is not available for this vehicle',
                  style: TextStyle(
                    fontSize: 11,
