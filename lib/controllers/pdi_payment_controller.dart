...
enum PdiPaymentSection { none, upi, cards, netBanking }

class PdiPaymentController extends GetxController {
  // Loading
  RxBool isWarrantyPaymentLoading = false.obs;

  // Dummy UI data
  final String warrantyLabel = 'New Car PDI + Visit Charge';
  final String priceSeparator = '-';
  final String warrantyPrice = '₹ 1,798';
  ...
  // State
  final Rx<PdiPaymentSection> expandedSection = PdiPaymentSection.upi.obs;
  ...
  void toggleSection(PdiPaymentSection section) {
    if (expandedSection.value == section) {
      expandedSection.value = PdiPaymentSection.none;
    } else {
...
class _Colors {
  ...
  final Color cardBg = const Color(0xFFB7C4D6);
